...
for(var i = json_iterate(global.leveldata,argument0,"MODIFIERS",ds_type_map); json_has_next(i); json_next(i))
{
    ...
    with instance_create(16*real(string_copy(i[JSONITER.KEY],0,string_pos(",",i[JSONITER.KEY]))),16*real(string_copy(i[JSONITER.KEY],string_pos(",",i[JSONITER.KEY])+1,string_length(i[JSONITER.KEY])-(string_pos(",",i[JSONITER.KEY])))),obj_editor_place_modifier)
    {
        if _objcontent != ""
            content = _objcontent
        myname = _objname
